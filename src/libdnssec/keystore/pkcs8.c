 ...
 */
static int key_open(const char *dir_name, const char *id, int flags, mode_t mode, int *fd_ptr)
{
	assert(dir_name);
	assert(id);
	assert(fd_ptr);
...
#ifdef ENABLE_OQS
	if (is_pqc_pem(pem)) {
		// Parse the OQS PEM directly — no GnuTLS involved
		int r = parse_pqc_pem(pem, &id, NULL, NULL);
		if (r != DNSSEC_EOK) {
			return r;
		}
		...
		assert(wrote_count == (ssize_t)pem->size);
		*id_ptr = id;
		return DNSSEC_EOK;
	}
#endif

	// retrieve key ID (traditional GnuTLS path)
...
_public_ int dnssec_keystore_init_pkcs8(dnssec_keystore_t **store_ptr)
{
